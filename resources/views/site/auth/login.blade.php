...
<head>
    ...
    <title>{{ __('site.auth.login_title') }} - {{ config('app.name', 'Meet2Be') }}</title>
        ...
        <div class="flex-1 flex justify-center items-center bg-stone-900">
            <div class="w-80 max-w-80 space-y-6">
                <div class="flex justify-center">
                    <a href="/" class="group flex items-center gap-3">
                        ...
                        <span class="text-xl font-semibold text-white">{{ config('app.name', 'Meet2Be') }}</span>
                                    ...
                                    <h1 class="text-center text-white text-2xl font-semibold">{{ __('site.auth.welcome_back') }}</h1>
                ...
                <form method="POST" action="{{ route('site.auth.login.store') }}" class="flex flex-col gap-6" x-data="loginForm()">
                    ...
                    >
                        <span x-show="!isSubmitting">{{ __('site.auth.login_button') }}</span>
                        <span x-show="isSubmitting" class="flex items-center justify-center">
                            <i class="fa-solid fa-spinner fa-spin mr-2"></i>
                            {{ __('site.auth.loading') }}
                        </span>
                    </button>
                </form>
                ...
                <div class="relative">
                    ...
                    <div class="relative flex justify-center text-sm">
                        <span class="px-2 bg-stone-900 text-stone-400">{{ __('site.auth.or') }}</span>
                    </div>
                </div>
                
                <p class="text-center text-stone-300 text-sm">
                    {{ __('site.auth.first_time') }} 
                    <a href="#" class="text-indigo-400 hover:text-indigo-300">{{ __('site.auth.sign_up_free') }}</a>
                </p>
            </div>
        </div>
            ...
            <div class="text-white relative rounded-lg h-full w-full overflow-hidden flex flex-col items-start justify-end p-16"
                ...
                <div class="relative z-10">
                    ...
                    <div class="mb-6 italic font-base text-3xl xl:text-4xl">
                        {{ __('site.testimonials.quote') }}
                    </div>
                    <div class="flex gap-4">
                        <img src="{{ asset('assets/images/site/ismail_celik.png') }}" alt="Prof. Dr. İsmail Çelik" class="h-14 w-14 rounded-full">
                        <div class="flex flex-col justify-center font-medium">
                            <div class="text-lg">{{ __('site.testimonials.author') }}</div>
                            <div class="text-stone-300">{{ __('site.testimonials.position') }}</div>
                        </div>
                    </div>
                </div>
            </div>
